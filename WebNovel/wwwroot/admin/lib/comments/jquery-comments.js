    ...
    var Comments = {
        ...
        getDefaultOptions: function () {
            return {
                ...
                oldestText: 'Cũ nhất',
                popularText: 'Nổi bật',
                attachmentsText: 'Tập tin đính kèm',
                sendText: 'Gửi',
                replyText: 'Trả lời',
                editText: 'Chỉnh sửa',
                ...
                noCommentsText: 'Không có tin nhắn...',
                loadMoreCommentsText: 'Xem thêm...',
                noAttachmentsText: 'Không có tập tin.',
                attachmentDropText: 'Drop files here',
                postOn: 'thêm một chủ đề trao đổi vào',
        ...
        init: function (options, el) {
            ...
            //if (self.options.commentContainer != '') {
            //    $(self.options.commentContainer).on('scroll', function () {
        ...
        createComments: function () {
            ...
            $(this.getComments()).each(function (index, commentModel) {
                if (commentModel.parent == null) {
                    ...
                    if (self.options.commentCount >= 10) {
        ...
        updateToggleAllButton: function (parentEl) {
            ...
            hiddenReplies.addClass('togglable-reply');
        ...
        prepareLader: function (commentsArray) {
            var self = this;
            if (self.options.commentCount >= 10 || (self.options.commentCount >= 10 && commentsArray.length >= 1)) {
                $.each(commentsArray, function (index, value) {
                    self.appendNewComment(value)
                });
            }
        ...
        createHTML: function () {
            ...
            if (this.options.enableAttachments) {
                ...
                var loadMore = $('<button/>', {
                    text: this.options.loadMoreCommentsText,
                    class: 'load-more',
                    id: 'load-more'
                });
        ...
        pingingUser: function (textarea) {
            var self = this;
            // Pinging users
            if (self.options.enablePinging) {
                ...
                $(".textarea").textcomplete([{
                    ...
                    template: function (user) {
                        ...
                        //yaman
                        var profilePictureEl = self.createProfilePictureElement(null, user.profile_picture_url);
        ...
        createCommentingFieldElement: function (parentId, existingCommentId, isMain) {
            ...
            var textareaWrapper = $('<div/>', {
                'class': this.options.isMainPage ? 'textarea-wrapper main-textarea-wrapper' : 'textarea-wrapper',
            });
            ...
            // Textarea
            var textarea = $('<div/>', {
                'class': this.options.isMainPage ? 'textarea main-textarea' : 'textarea',
        ...
        createCommentElement: function (commentModel) {
            ...
            var commentEl = $('<li/>', {
                ...
                'class': 'comment' + (commentModel.parent == null ? ' comment-panel' : '')
        ...
        createCommentWrapperElement: function (commentModel) {
            var commentWrapper = $('<div/>', {
                'class': 'comment-wrapper ' + (commentModel.parent == null ? 'main-comment' : ''),
            });
            ...
            var postOn = $('<span/>', {
                'text': " " + this.options.postOn + " ",
                'class': "post-on-text"
            }).append(" ").append($('<a/>', {
                'href': this.options.rootUrl + commentModel.postOnURL,
                'html': commentModel.groupName,
                'class': "post-on-link"
            }));
            ...
            if (commentModel.parent || !(commentModel.postOnURL != null && commentModel.postOnURL != undefined && commentModel.postOnURL.length >= 5)) {
                var nameEl = $('<div/>', {
                    'class': 'name',
                    'html': name
                });
            ...
            if (isAttachment) {
                ...
                content.html("<p class='comment-text'>" + this.highlightTags(commentModel, this.linkify(this.escape(commentModel.content)), commentModel.id) + ' </p>');
            ...
            if (commentModel.createdByCurrentUser || this.options.currentUserIsAdmin) {
                ...
                //edit by man
                if (isAttachment && commentModel.createdByCurrentUser) {//this.isAllowedToDelete(commentModel.id)
                ...
                } else if (!isAttachment && commentModel.createdByCurrentUser) {// this.options.enableEditing
        ...
        getOutermostParent: function (directParentId) {
            ...
            do {
                ...
                if (parentComment != undefined && parentComment.parent != undefined)
                    parentId = parentComment.parent;
            } while (parentComment != null && parentComment.parent != null);
        ...
        getFormattedCommentContent: function (commentModel, replaceNewLines) {
            ...
            html = this.linkify(html, commentModel.id);
        ...
        getPings: function (textarea) {
            let tempPings = [];
            $.map(textarea.find('.ping'), function (el) {
                // console.log($(el).attr('data-value'));
                tempPings.push($(el).attr('data-value'));
        ...
        highlightHashtags: function (commentModel, html) {
            ...
            if (html.indexOf('#') != -1) {
                ...
                var regex = /(^|\s)#([A-Za-zا-ي\d-_أىäöüß]+)/ig;
        ...
        highlightPings: function (commentModel, html) {
            ...
            if (html.indexOf('@') != -1) {
                var __createTag = function (tag) {
                    ...
                    var tag = self.createTagElement('@' + tag, 'ping', tag);
                    return tag[0].outerHTML;
                }

                var regex = /(^|\s)@([a-z\d-_.]+|[A-Z\d-_.]+|[أ-ي\d-_.]+||[ا-ي\d-_.]+)/gim;
                html = html.replace(regex, function ($0, $1, $2) {
                    return $1 + __createTag($2);
                });
            }
        ...
        getYoutubeEmbededId: function (url) {
        ...
        linkify: function (inputText, id) {
            var replacedText, replacePattern1, replacePattern2, replacePattern3;

            if (this.matchYoutubeUrl(inputText)) {
                let description = inputText.replace(/(?:https?|ftp):\/\/[\n\S]+/g, '');
                // URLs starting with http://, https://, ftp:// or file://
                replacePattern1 = /(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/gim;

                inputText = this.getYoutubeEmbededId(inputText)
                replacedText = inputText.replace(replacePattern1, '<iframe  width="420" height="315" src="$1" >$1</iframe>');
                ...
                replacedText = replacedText.replace(replacePattern2, '$1<iframe   width="420" height="315" src="http://$2" >$2</iframe>');

                replacedText = '<p>' + description + '</p>' + replacedText;
            }
            else {
                // URLs starting with http://, https://, ftp:// or file://
                replacePattern1 = /(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/gim;
                replacedText = inputText.replace(replacePattern1, '<a id="link-preview-' + id + '" class="link-preview" href="$1" target="_blank">$1</a>');

                // URLs starting with "www." (without // before it, or it would re-link the ones done above).
                replacePattern2 = /(^|[^\/f])(www\.[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/gim;
                replacedText = replacedText.replace(replacePattern2, '$1<a id="link-preview-' + id + '" class="link-preview" href="http://$2" target="_blank">$2</a>');

                // Change email addresses to mailto: links.
                replacePattern3 = /(([a-zA-Z0-9\-\_\.])+@[a-zA-Z\_]+?(\.[a-zA-Z]{2,6})+)/gim;
                replacedText = replacedText.replace(replacePattern3, '<a id="link-preview-' + id + '" class="link-preview" href="mailto:$1">$1</a>');
            }
