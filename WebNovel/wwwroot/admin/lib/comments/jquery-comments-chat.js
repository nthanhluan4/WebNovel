...
(function (factory) {
    ...
    var ChatComments = {
        ...
        events: {
            ...
            'click .chat-commenting-field.main .textarea': 'showMainCommentingField',
            'click .chat-commenting-field.main .close': 'hideMainCommentingField',

            // All commenting fields
            'click .chat-commenting-field .textarea': 'increaseTextareaHeight',
            'change .chat-commenting-field .textarea': 'increaseTextareaHeight textareaContentChanged',
            'click .chat-commenting-field:not(.main) .close': 'removeCommentingField',

            // Edit mode actions
            'click .chat-commenting-field .send.enabled': 'postComment',
            'click .chat-commenting-field .update.enabled': 'putComment',
            'click .chat-commenting-field .delete.enabled': 'deleteComment',
            'change .chat-commenting-field .upload.enabled input[type="file"]': 'fileInputChanged',

            // Other actions
            'click li.chat-comment button.upvote': 'upvoteComment',
            'click li.chat-comment button.delete.enabled': 'deleteComment',
            'click li.chat-comment .hashtag': 'hashtagClicked',
            'click li.chat-comment .ping': 'pingClicked',

            // Other
            'click li.chat-comment ul.chat-child-comments .toggle-all': 'toggleReplies',
            'click li.chat-comment button.reply': 'replyButtonClicked',
            'click li.chat-comment button.edit': 'editButtonClicked',
        ...
        getDefaultOptions: function () {
            return {
                ...
                textareaPlaceholderText: 'Thêm một bình luận',
                newestText: 'Mới nhất',
                oldestText: 'Cũ nhất',
                popularText: 'Nổi bậc',
                ...
                deleteText: 'Xóa',
                newText: 'New',
                viewAllRepliesText: 'Xem tất cả __replyCount__ replies',
                hideRepliesText: 'Ẩn phản hồi',
                noCommentsText: 'Không có nội dung',
                noAttachmentsText: 'Không có tập tin đính kèm',
                attachmentDropText: 'Thả tập tin tại đây',
                ...
                timeFormatterNew: function (time) {
                    var date = new Date(time);
                    // Lấy giờ và phút
                    var hours = date.getHours().toString().padStart(2, '0'); // Đảm bảo luôn có 2 chữ số
                    ...
                    // Định dạng thành HH:mm, dd Thg MM yyyy
        ...
        init: function (options, el) {
            ...
            this.$el = $(el);
            this.$el.addClass('jquery-chat-comments');
            ...
            $($(el).selector).scroll(function () {
                if ($($(el).selector).scrollTop() - 150 <= $($(el).selector).height() - $($(el).selector).height()) {
                    self.options.loadMoreComments(self)
                }
            });
            ...
            $(function () {
                window.emojiPicker = new EmojiPicker({
                    ...
                    assetsPath: self.options.rootUrl + '/lib/emoji-picker/img/',
        ...
        appendNewComment: function (commentJSON) {
            var self = this;
            var commentModel = self.createCommentModel(commentJSON);
            self.addCommentToDataModel(commentModel);
            self.appendComment(commentModel);
        },
        prependNewComment: function (commentJSON) {
            var self = this;
            var commentModel = self.createCommentModel(commentJSON);
            self.addCommentToDataModel(commentModel);
            self.prependComment(commentModel);
        },
        prependComment: function (commentModel, commentList) {
            commentList = commentList || this.$el.find('#comment-list');
            var commentEl = this.createCommentElement(commentModel);

            // Case: reply
            if (commentModel.parent) {
                var directParentEl = commentList.find('.chat-comment[data-id="' + commentModel.parent + '"]');

                // Re-render action bar of direct parent element
                this.reRenderCommentActionBar(commentModel.parent);

                // Force replies into one level only
                var outerMostParent = directParentEl.parents('.chat-comment').last();
                if (outerMostParent.length == 0) outerMostParent = directParentEl;

                // Append element to DOM
                var childCommentsEl = outerMostParent.find('.chat-child-comments');
                var commentingField = childCommentsEl.find('.chat-commenting-field');
                ...
                this.options.commentCount++;
                commentList.prepend(commentEl);
            }
        }
        ,
        appendComment: function (commentModel, commentList) {
            commentList = commentList || this.$el.find('#comment-list');
            var commentEl = this.createCommentElement(commentModel);
        ...
        removeComment: function (commentId) {
            ...
            var commentElements = this.$el.find('li.chat-comment[data-id="' + commentId + '"]');
            var parentEl = commentElements.parents('li.chat-comment').last();
        ...
        uploadAttachments: function (files, commentingField) {
            var self = this;
            if (!commentingField) commentingField = this.$el.find('.chat-commenting-field.main');
            ...
            if (fileCount) {
                ...
                $(files).each(function (index, file) {
                    ...
                    commentJSON.fileURL = file.name;//'C:/fakepath/' +file.name;
        ...
        updateToggleAllButton: function (parentEl) {
            ...
            var childCommentsEl = parentEl.find('.chat-child-comments');
            var childComments = childCommentsEl.find('.chat-comment');
        ...
        sortAndReArrangeComments: function (sortKey) {
            ...
            $(mainLevelComments).each(function (index, commentModel) {
                var commentEl = commentList.find('> li.chat-comment[data-id=' + commentModel.id + ']');
                commentList.append(commentEl);
            });
        },
        ...
        textareaContentChanged: function (ev) {
            ...
            if (!textarea.find('.reply-to.tag').length) {
                ...
                if (commentId) {
                    var parentComments = textarea.parents('li.chat-comment');
                    ...
                    var parentId = textarea.parents('li.chat-comment').last().data('id');
                    textarea.attr('data-parent', parentId);
                }
            }

            // Move close button if scrollbar is visible
            var commentingField = textarea.parents('.chat-commenting-field').first();
        ...
        removeCommentingField: function (ev) {
            ...
            if (textarea.attr('data-comment')) {
                closeButton.parents('li.chat-comment').first().removeClass('edit');
            }

            // Remove the field
            var commentingField = closeButton.parents('.chat-commenting-field').first();
        ...
        postComment: function (ev) {
            ...
            var commentingField = sendButton.parents('.chat-commenting-field').first();
            var textarea = commentingField.find('.textarea');
        ...
        putComment: function (ev) {
            ...
            var commentingField = saveButton.parents('.chat-commenting-field').first();
            var textarea = commentingField.find('.textarea');
        ...
        deleteComment: function (ev) {
            ...
            if (confirm("Are you sure to do this action?")) {
                ...
                var commentEl = deleteButton.parents('.chat-comment').first();
        ...
        fileInputChanged: function (ev, files) {
            ...
            var commentingField = $(ev.currentTarget).parents('.chat-commenting-field').first();
            this.uploadAttachments(files, commentingField);
        },

        upvoteComment: function (ev) {
            var self = this;
            var commentEl = $(ev.currentTarget).parents('li.chat-comment').first();
        ...
        replyButtonClicked: function (ev) {
            ...
            var outermostParent = replyButton.parents('li.chat-comment').last();
            var parentId = replyButton.parents('.chat-comment').first().data().id;


            // Remove existing field
            var replyField = outermostParent.find('.chat-child-comments > .chat-commenting-field');
            ...
            if (previousParentId != parentId) {
                ...
                outermostParent.find('.chat-child-comments').append(replyField);
        ...
        editButtonClicked: function (ev) {
            ...
            var commentEl = editButton.parents('li.chat-comment').first();
            var commentModel = commentEl.data().model;
            commentEl.addClass('edit');

            // Create the editing field
            var editField = this.createCommentingFieldElement(commentModel.parent, commentModel.id);
            commentEl.find('.chat-comment-wrapper').first().append(editField);
        ...
        createProfilePictureElement: function (src, userId) {
            if (userId) {
                var profilePicture = $('<div/>').css({
                    'background-image': 'url(' + this.options.commentProfilePicturePath + userId + ')'
                });
            } else {
        ...
        createCommentingFieldElement: function (parentId, existingCommentId, isMain) {
            ...
            var commentingField = $('<div/>', {
                'class': 'chat-commenting-field'
            });

            if (!this.options.enableCommentingField && isMain)
                commentingField.css("display", 'none');
        ...
        createCommentElement: function (commentModel) {
            ...
            var commentEl = $('<li/>', {
                ...
                'class': 'chat-comment' + (commentModel.parent == null ? ' chat-comment-panel' : '')
        ...
        createCommentWrapperElement: function (commentModel) {
            var commentWrapper = $('<div/>', {
                'class': 'chat-comment-wrapper ' + (commentModel.parent == null ? 'main-chat-comment' : ''),
            });

            // Profile picture
            var profilePicture = this.createProfilePictureElement(commentModel.profilePictureURL, commentModel.creator);

            var timeStr = this.options.timeFormatterNew(commentModel.created);
            // Time
            var time = $('<time/>', {
                text: timeStr,
            ...
            if (isAttachment) {
                ...
                var link = $('<a/>', {
                    'class': 'attachment',
                    href: this.options.filePath + commentModel.id,
                    target: '_blank'
                });
                ...
                if (type == 'image') {
                    var image = $('<img/>', {
                        src: this.options.filePath + commentModel.id
                    });
                    link.html(image);
                    ...
                    var callDownloadAttachment = function () {
                        self.options.downloadAttachment(commentModel)
                    };
                    link = $('<a/>', {
                        'class': 'attachment',
                        ...
                        target: '_blank',
                        id: self.options.filePath + commentModel.id,
                        click: function () {
                            callDownloadAttachment();
                        }
                    });
                ...
                content.html("<p class='comment-text'>" + this.highlightTags(commentModel, this.linkify(this.escape(commentModel.content))) + ' </p>');
            ...
            if (commentModel.modified && commentModel.modified != commentModel.created) {
                ...
                if (editedTime !== null && editedTime !== '' && editedTime.localeCompare("1/1/1") !== 0) {
            ...
            if (commentModel.createdByCurrentUser || this.options.currentUserIsAdmin) {
                ...
                //edit by man
                if (isAttachment && commentModel.createdByCurrentUser && this.options.enableDeleting) {//this.isAllowedToDelete(commentModel.id)
                ...
                } else if (!isAttachment && commentModel.createdByCurrentUser && this.options.enableEditing) {// this.options.enableEditing
            ...
            var rightCtHeader = $('<div/>', {
                'class': 'right-content-header'
            });
            rightCtHeader.append(nameEl).append(time)
            rightContent.append(rightCtHeader).append(wrapper);
            commentWrapper.append(profilePicture).append(rightContent);

            //commentWrapper.append(profilePicture).append(nameEl).append(time).append(wrapper);
        ...
        reRenderComment: function (id) {
            ...
            var commentElements = this.$el.find('li.chat-comment[data-id="' + commentModel.id + '"]');

            var self = this;
            commentElements.each(function (index, commentEl) {
                var commentWrapper = self.createCommentWrapperElement(commentModel);
                $(commentEl).find('.chat-comment-wrapper').first().replaceWith(commentWrapper);
            });
        },
        ...
        createCssDeclarations: function () {
            ...
            $('head style.jquery-chat-comments-css').remove();

            // Navigation underline
            this.createCss('.jquery-chat-comments ul.navigation li.active:after {background: '
                + this.options.highlightColor + ' !important;',
                +'}');

            // Dropdown active element
            this.createCss('.jquery-chat-comments ul.navigation ul.dropdown li.active {background: '
                + this.options.highlightColor + ' !important;',
                +'}');

            // Background highlight
            this.createCss('.jquery-chat-comments .highlight-background {background: '
                + this.options.highlightColor + ' !important;',
                +'}');

            // Font highlight
            this.createCss('.jquery-chat-comments .highlight-font {color: '
                + this.options.highlightColor + ' !important;'
                + '}');
            this.createCss('.jquery-chat-comments .highlight-font-bold {color: '
                + this.options.highlightColor + ' !important;'
                + 'font-weight: bold;'
                + '}');
        },
        ...
        createCss: function (css) {
            var styleEl = $('<style/>', {
                type: 'text/css',
                'class': 'jquery-chat-comments-css',
                text: css
            });
            $('head').append(styleEl);
        },
        ...
        setToggleAllButtonText: function (toggleAllButton, toggle) {
            ...
            var showExpandingText = function () {
                ...
                var replyCount = toggleAllButton.siblings('.chat-comment').length;
        ...
        getTextareaContent: function (textarea, humanReadable) {
            ...
            // Trim leading spaces
            var text = textareaClone.text().replace(/^\s+/g, '');
        ...
        highlightHashtags: function (commentModel, html) {
            ...
            if (html.indexOf('#') != -1) {
                ...
                var regex = /(^|\s)#([a-zا-ي أى äöüß\d-_]+)/gim;
        ...
        applyMappings: function (mappings, commentJSON) {
            ...
            for (var key1 in commentJSON) {
                ...
                var lowKey1 = key1.charAt(0).toLowerCase() + key1.slice(1);
                if (lowKey1 in mappings) {
                    var key2 = mappings[lowKey1];
    ...
    $.fn.chatComments = function (options) {
        var chatComments = Object.create(ChatComments);
        $.data(this, 'chatComments', chatComments);
        chatComments.init(options || {}, this);
        return chatComments;
    };
}));
