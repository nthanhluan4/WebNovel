...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        // Lấy giá trị văn hóa mặc định từ localStorage hoặc thiết lập mặc định là 'en-US'
        var defaultCulture = localStorage.getItem('kendoCulture') || 'vi-VN';

        // Thiết lập văn hóa Kendo theo giá trị từ localStorage
        kendo.culture(defaultCulture);

        // Đặt radio button dựa trên văn hóa đã lưu trong localStorage
        document.querySelector(`input[name="data-culture"][value="${defaultCulture}"]`).checked = true;

        // Lắng nghe sự kiện thay đổi cho các nút radio
        const cultureRadios = document.getElementsByName('data-culture');
        cultureRadios.forEach(function (radio) {
            radio.addEventListener('change', function (event) {
                const selectedCulture = event.target.value;
                if (selectedCulture === 'vi-VN') {
                    // Thiết lập văn hóa Kendo UI cho Việt Nam
                    kendo.culture("vi-VN");
                } else {
                    // Thiết lập văn hóa mặc định (English)
                    kendo.culture("en-US");
                }

                // Lưu văn hóa đã chọn vào localStorage
                localStorage.setItem('kendoCulture', selectedCulture);

                // Refresh lại các thành phần Kendo để áp dụng văn hóa mới
    ...
    function twoColumnMenuGenerate() {
        ...
        if (defaultValues && (isTwoColumn == "twocolumn" || defaultValues["data-layout"] == "twocolumn")) {
            ...
            ul.innerHTML = '<a href="#" class="logo"><img src="/assets/images/logo-gold.png" alt="" height="22"></a>';
        ...
        if (document.documentElement.getAttribute("data-layout") === "vertical") {
            if (windowSize <= 1025 && windowSize > 767) {
                ...
                if (document.documentElement.getAttribute("data-sidebar-size") == "sm") {
                    ...
                    localStorage.setItem("data-sidebar-size", "");
                } else {
                    document.documentElement.setAttribute("data-sidebar-size", "sm");
